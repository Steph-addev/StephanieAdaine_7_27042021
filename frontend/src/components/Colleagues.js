        ...
        <div>
          {["right"].map((anchor) => (
            <React.Fragment key={anchor}>
              <Button onClick={toggleDrawer(anchor, true)} className="btn-sidebars justify-content-start text-lowercase">
                <FaAngleDoubleLeft className="me-1" /> Ouvrir le chat
              </Button>
      ...
      <MediaQuery minWidth={426}>
        <div className="colleagues">
          <div className="colleagues-list">
            ...
            <ul className="p-0">
              {users.map((param) => (
                <li key={param.id} post={param} className="colleagues-list_data p-1 d-flex flex-wrap">
                  <Avatar src={param.profileImage ? param.profileImage : PF + "profile-picture.png"} className="colleagues-list_data--profilePic" alt="profil pic"></Avatar>
                  {param.username}
                </li>
              ))}
